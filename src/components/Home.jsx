...
import { programmingLanguageLogos, publicUrl, socialMediaIcons } from '../api'
    ...
    return (
        <section className='w-full h-screen relative overflow-hidden flex items-center justify-center' onMouseMove={parallax}>
            {logos.map(ele => <div key={ele.logo} className='w-screen h-screen absolute top-0 left-0' style={{ translate: `${ele.xAxis}px ${ele.yAxis}px` }} >
                <img src={`${publicUrl}/icons/${ele.logo}.svg`} alt={ele.logo} style={{ width: '50px', translate: `${ele.xAxisPermenent}vw ${ele.yAxisPermenent}vh`, opacity: '.5' }} />
            </div>)}

            {/* <img src="./public/icons/logo.svg" alt="logo" className='absolute w-[400px]' /> */}
            <img src={`${publicUrl}/icons/logo.svg`} alt="logo" className='absolute w-[400px]' />
            ...
            <div className='absolute bottom-10 flex items-center flex-col gap-2 group'>
                ...
                <img src={`${publicUrl}/icons/arrow.svg`} alt="arrow" className='w-[25px] absolute top-8 group-hover:top-12 ease-in-out duration-300' />
            </div>
        </section>
    )
}